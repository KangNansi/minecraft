...
class NYChunk
{
	public :
				...
				for(int y=0; y<CHUNK_SIZE; y++)
					for (int z = 0; z < CHUNK_SIZE; z++) {
						if (_Cubes[x][y][z]._Draw) {
							...
							case CUBE_EAU:
								vboAddCube(x, y, z, NYColor(0.2, 0.5, 1, 0.6), false);
								break;
							case CUBE_TERRE:
								vboAddCube(x, y, z, NYColor(0.4+randf()/4.0, 0.2, 0.05, 1));
								break;
							case CUBE_HERBE:
								vboAddCube(x, y, z, NYColor(0.2, 1 + randf() / 8.0, 0.2, 1));
								break;
							default:
								break;
							}
		...
		void vboAddCube(int x, int y, int z, NYColor color, bool opaque = true) {
			...
			//Face X pos
			if (cubeXNext==nullptr 
				|| (_Cubes[x][y][z]._Type != cubeXNext->_Type && !cubeXNext->isSolid())) {
				addVertice(NYVert3Df(x+1, y, z), color, NYVert3Df(1, 0, 0), opaque);
				addVertice(NYVert3Df(x+1, y+1, z), color, NYVert3Df(1, 0, 0), opaque);
				addVertice(NYVert3Df(x+1, y+1, z+1), color, NYVert3Df(1, 0, 0), opaque);
				addVertice(NYVert3Df(x+1, y, z+1), color, NYVert3Df(1, 0, 0), opaque);
			}
			//Face Y pos
			if (cubeYNext == nullptr
				|| (_Cubes[x][y][z]._Type != cubeYNext->_Type && !cubeYNext->isSolid())) {
				addVertice(NYVert3Df(x, y + 1, z), color, NYVert3Df(0, 1, 0), opaque);
				addVertice(NYVert3Df(x, y + 1, z + 1), color, NYVert3Df(0, 1, 0), opaque);
				addVertice(NYVert3Df(x + 1, y + 1, z + 1), color, NYVert3Df(0, 1, 0), opaque);
				addVertice(NYVert3Df(x + 1, y + 1, z), color, NYVert3Df(0, 1, 0), opaque);
			}
			//Face Z pos
			if (cubeZNext == nullptr
				|| (_Cubes[x][y][z]._Type != cubeZNext->_Type && !cubeZNext->isSolid())) {
				addVertice(NYVert3Df(x, y, z + 1), color, NYVert3Df(0, 0, 1), opaque);
				addVertice(NYVert3Df(x + 1, y, z + 1), color, NYVert3Df(0, 0, 1), opaque);
				addVertice(NYVert3Df(x + 1, y + 1, z + 1), color, NYVert3Df(0, 0, 1), opaque);
				addVertice(NYVert3Df(x, y + 1, z + 1), color, NYVert3Df(0, 0, 1), opaque);
			}
			//Face X neg
			if (cubeXPrev == nullptr
				|| (_Cubes[x][y][z]._Type != cubeXPrev->_Type && !cubeXPrev->isSolid())) {
				addVertice(NYVert3Df(x, y, z), color, NYVert3Df(-1, 0, 0), opaque);
				addVertice(NYVert3Df(x, y, z + 1), color, NYVert3Df(-1, 0, 0), opaque);
				addVertice(NYVert3Df(x, y + 1, z + 1), color, NYVert3Df(-1, 0, 0), opaque);
				addVertice(NYVert3Df(x, y + 1, z), color, NYVert3Df(-1, 0, 0), opaque);
			}
			//Face Y neg
			if (cubeYPrev == nullptr
				|| (_Cubes[x][y][z]._Type != cubeYPrev->_Type && !cubeYPrev->isSolid())) {
				addVertice(NYVert3Df(x, y, z), color, NYVert3Df(0, -1, 0), opaque);
				addVertice(NYVert3Df(x + 1, y, z), color, NYVert3Df(0, -1, 0), opaque);
				addVertice(NYVert3Df(x + 1, y, z + 1), color, NYVert3Df(0, -1, 0), opaque);
				addVertice(NYVert3Df(x, y, z + 1), color, NYVert3Df(0, -1, 0), opaque);
			}
			//Face Z neg
			if (cubeZPrev == nullptr
				|| (_Cubes[x][y][z]._Type != cubeZPrev->_Type && !cubeZPrev->isSolid())) {
				addVertice(NYVert3Df(x, y, z), color, NYVert3Df(0, 0, -1), opaque);
				addVertice(NYVert3Df(x, y + 1, z), color, NYVert3Df(0, 0, -1), opaque);
				addVertice(NYVert3Df(x + 1, y + 1, z), color, NYVert3Df(0, 0, -1), opaque);
				addVertice(NYVert3Df(x + 1, y, z), color, NYVert3Df(0, 0, -1), opaque);
			}
		}

		void addVertice(NYVert3Df v, NYColor c, NYVert3Df n, bool opaque = true) {
			int position = (opaque?_NbVertices*3:_NbVerticesT*3);
			float *vert, *cols, *norm;
