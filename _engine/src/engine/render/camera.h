...
class NYCamera
{
	public:
		...
		void moveNormal(float distance) {
		...
		{
			NYVert3Df npos = _LookAt - _Position;
			npos.rotate(_UpRef, angle);
			_LookAt = _Position + npos;
			updateVecs();
		}
		...
		{
			NYVert3Df npos = _Position - _LookAt;
			npos.rotate(_UpRef, angle);
			_Position = _LookAt + npos;
			updateVecs();
		}
